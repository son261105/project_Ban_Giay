    ...
    <div style={{
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <h2 style={{ margin: 0, fontSize: 20 }}>Chi tiết người dùng</h2>
            <p style={{ margin: '4px 0 0', color: '#888', fontSize: 13 }}>Thông tin tài khoản và lịch sử mua hàng</p>
          </div>
        ...
        <h3 style={{ fontSize: 14, marginBottom: 14 }}>Thông tin tài khoản</h3>
        ...
        <h3 style={{ fontSize: 14, marginBottom: 14 }}>Thông tin mua hàng</h3>
        {loadingSummary ? (
          <div style={{ color: '#888', fontSize: 14 }}>Đang tải...</div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div>
        ...
        <h3 style={{ fontSize: 14, marginBottom: 14 }}>Đơn hàng gần đây</h3>
        {!loadingSummary && summary.recentOrders.length === 0 && (
          <div style={{ color: '#888', fontSize: 14 }}>Chưa có đơn hàng nào.</div>
        )}
        {!loadingSummary && summary.recentOrders.length > 0 && (
    ...
    <AdminLayout>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
        <input
          ...
          style={{ padding: '10px 16px', border: '2px solid var(--border)', borderRadius: 10, fontSize: 14, width: 280 }}
        />
        <select
          value={roleFilter}
          onChange={e => setRoleFilter(e.target.value)}
          style={{ padding: '10px 16px', border: '2px solid var(--border)', borderRadius: 10, fontSize: 14 }}
        >
        ...
        <div className="table-container">
          ...
          {filtered.length > 0 && (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 4px 4px' }}>
              <span style={{ color: '#888', fontSize: 13 }}>{PAGE_SIZE} người dùng / trang</span>
